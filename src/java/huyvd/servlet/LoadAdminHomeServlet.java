 ...
 * @author BlankSpace
 */
@WebServlet(name = "LoadAdminHomeServlet", urlPatterns = {"/LoadAdminHomeServlet"})
public class LoadAdminHomeServlet extends HttpServlet {
	...
	try {
	    ...
	    List<TblQuestionDTO> questionList = questionDAO.loadQuestion();
	    request.setAttribute("QUESTION_LIST", questionList);
	    
	    //calculate total page for paging
	    int totalQuestions = questionDAO.getTotalQuestion();
	    int totalPages = PageCalculate.calculateNumOfPage(totalQuestions, 5);
	    request.setAttribute("TOTAL_PAGES", totalPages);
	    request.setAttribute("DEFAULT_PAGE_INDEX", 1);
	} catch (SQLException e) {
	    log("LoadAdminHomeServlet_SQLException" + e.getMessage());
	} catch (NamingException e) {
	    log("LoadAdminHomeServlet_NamingException" + e.getMessage());
	} finally {
